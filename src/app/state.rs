...
pub struct AppState {
    ...
    /// Foreground executables that keep the directional pane keys for
    /// themselves. See `KeysConfig`.
...
impl AppState {
    ...
    pub(crate) fn space_active_bg(&self) -> Color {
        self.theme_runtime
            .space_active_bg
            .unwrap_or(self.palette.active_row_bg)
    }
    ...
    /// `default` is upstream's resolved selection background, which depends on
    /// whether the row is also the active space.
    pub(crate) fn space_selected_bg(&self, default: Color) -> Color {
        self.theme_runtime.space_selected_bg.unwrap_or(default)
    }
