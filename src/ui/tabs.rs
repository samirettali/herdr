...
// `[ui.tab_bar] min_width` is configurable, but the strip threshold has to be a
// constant, so it keeps assuming the default tab width.
const MIN_TAB_WIDTH: u16 = crate::config::DEFAULT_TAB_MIN_WIDTH;
...
pub(super) fn render_tab_bar(app: &AppState, frame: &mut Frame, area: Rect) {
    ...
    for (idx, tab) in ws.tabs.iter().enumerate() {
        ...
        // `label_padding` sets the tab's width, and centring spends whatever
        // `min_width` adds on top of it evenly on both sides.
...
mod tests {
    ...
    fn cjk_tab_labels_are_centered_by_display_width() {
        ...
        // Centring is what this asserts, so drop the index the default label
        // prepends and leave the name alone.
        app.tab_bar.label = vec![crate::config::TabBarToken::Name];
        app.workspaces = vec![ws];
        app.active = Some(0);
        app.view.tab_bar_rect = Rect::new(0, 0, 30, 1);
        let view = compute_tab_bar_view(
