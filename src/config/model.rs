...
pub struct KeysConfig {
    ...
    /// Open the tab navigation surface: the same mode as `workspace_picker`,
    /// moving over the tab rows of the tree sidebar. Unset by default.
    pub tab_picker: BindingConfig,
...
pub(crate) struct KeysConfigOverlay {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    tab_picker: Option<BindingConfig>,
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl KeysConfig {
    ...
    pub(crate) fn local_profile(&self, keybinds: &Keybinds) -> KeysConfigOverlay {
        ...
        copy_effective_action_field!(tab_picker, keybinds.tab_picker);
...
impl Default for KeysConfig {
    fn default() -> Self {
        Self {
            ...
            tab_picker: BindingConfig::empty(),
...
"#,
    ...
    #[test]
    fn passthrough_commands_default_empty_and_parse() {
        assert!(Config::default().keys.passthrough_commands.is_empty());
        let config: Config = toml::from_str(
            r#"
[keys]
passthrough_commands = ["nvim", "vim"]
"#,
        )
        .unwrap();
        assert_eq!(config.keys.passthrough_commands, ["nvim", "vim"]);
    }
